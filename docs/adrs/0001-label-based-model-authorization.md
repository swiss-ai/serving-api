...
- 403 (not 404) for unauthorized access deliberately trades a small information leak — the model id exists — for a debuggable error. The leak is bounded: since [ADR-0002](0002-username-namespaced-served-model-names.md) an id reveals the launcher's username and the catalogue model, both of which `launched_by` and the public model list already expose.
