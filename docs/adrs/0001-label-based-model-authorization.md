...
A refused name is **out of service**, and the listing says so rather than leaving the caller to discover it one 403 at a time: every entry of a conflicted name carries `authorization_conflict`, and the model card shows it as unavailable — red status dot, "Out of service" badge, an inert Chat button, and one line on the collapsed tile saying that separate launches disagree about who may use it. "Manage access" stays live, since that is where an owner resolves it.

The gateway computes that flag over **all** launches serving the name, before either listing filter runs, and the frontend does not second-guess it. Both filters hide entries that are still routable and still collide: `platform_namespaced` drops entries for advertising reasons (an old OpenTela, a below-minimum SML), and per-caller filtering hides the other launch from anyone its policy excludes — which is most people, since a disagreement usually means one side is restricted. A client-side comparison of the payload would therefore go quiet in exactly the case that matters most: the caller sees one healthy-looking launch and every request to it fails.
...
- Display names are only as good as the IdP claim behind them. A user who has never loaded the web UI has no `owner_name` row, and their launches read as a name derived from the local part ("john.doe2@ethz.ch" → "John Doe", "s1234567@student.ethz.ch" → "S1234567"). Names are for display only: every decision the UI makes about a model — who may manage it, whether it is out of service — is reported by the gateway as its own field, so two users with the same display name change nothing but what is printed.
