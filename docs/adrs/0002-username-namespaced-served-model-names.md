...
Every model on the OpenTela mesh is addressed by its served name, and that name is a flat, first-come string. Two consequences fell out of ADR-0001:

- **Collisions are ordinary, not exceptional.** Two users launching the same catalogue model naturally pick the same name. ADR-0001 made a collision with differing `authorization` labels a hard 403 for *everyone*, so an ordinary name clash became an outage for both launches.
...
Two launches by the *same* user of the same model do still share a name. That is deliberate: they carry identical labels, so no conflict is triggered and OpenTela simply load-balances across them as extra replicas — which is what a user relaunching to add capacity wants.

### The gateway cross-checks the namespace against `launched_by`

A peer serving `alice/swiss-ai/X` from a job that ran as `bob` is publishing under a namespace that is not its own. The gateway refuses it:

- **Listing** — `/v1/models` and `/v1/models_detailed` drop that peer's entry. It is never advertised.
- **Routing** — `ensure_model_access` returns 403 for the id if *any* peer serving it fails the check, exactly as with a policy conflict and for the same reason: OpenTela balances a name across every peer advertising it, so the gateway cannot keep a request off the squatting peer.
...
- Peers advertising no `launched_by` at all (OpenTela <v0.0.6 emits no labels) — unchecked, matching ADR-0001's "missing label ⇒ back-compatible" stance.

Comparison is case-insensitive and whitespace-tolerant on both sides, like every other label comparison in the service.
...
- **Ordinary cross-user collisions disappear.** ADR-0001's deny-all conflict rule now fires only for genuine same-user policy disagreement or deliberate squatting, not for two people launching the same catalogue model on the same day.
- **This is a consistency check, not an authentication boundary.** Both the served name and `launched_by` are self-asserted by whoever joins the mesh; a peer that sets `launched_by=alice` *and* serves `alice/...` passes. It catches accidental squats, stale hand-written scripts, and namespace-changing attacks — the same class ADR-0001's conflict detection covers, and with the same residual hole. Closing it needs authenticated labels / mesh-level name ownership in OpenTela.
...
2. **Namespace by email instead of cluster username.** Rejected: the mesh has no email; the SLURM job knows `$USER` and nothing else. Emails would have to be resolved at submission (as `--authorization private` is) and could not be cross-checked against any label the job emits.

3. **Build the name in bash from `$USER` at job start**, guaranteeing it matches the `launched_by` label byte for byte. Rejected: SML must know the served name at submission time — it is what the CLI prints, what health checks poll, what the MCP tool returns, and what the telemetry labels carry. Resolving the username in Python (`Launcher.username`, which is the FirecREST/SLURM account) and letting the gateway verify it against the label gets the same guarantee without a name that only exists after the job starts.

4. **No gateway check — rename only.** Rejected: a namespace nobody verifies is decoration. The check is cheap (the map the authorization service already builds per TTL) and turns a squat into a visible, attributable error.
...
- Builds on [ADR-0001](0001-label-based-model-authorization.md) (label-based authorization, deny-all on conflict).
