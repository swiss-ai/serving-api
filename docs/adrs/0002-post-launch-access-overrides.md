...
- **No lifecycle to observe.** A row whose launch is gone is *inert*, not stale: nothing can ever present that `launch_id` again, so a dead row cannot attach to anything. Pruning (`prune_dead_overrides`) is housekeeping, not correctness — and because that is the whole of its value, it deletes a row only after its launch has been *continuously* absent from the mesh for a grace window (24h), marking rather than deleting on first sight. A single DNT read can be partial (a heartbeat gap, a mesh mid-convergence), and pruning on one would revert that model to its launch-time label: for the common "launched public, then restricted it" case, a private model silently goes public again. The grace clock lives in the shared cache, so losing it restarts the window — it can only ever delay a delete, never cause one.
